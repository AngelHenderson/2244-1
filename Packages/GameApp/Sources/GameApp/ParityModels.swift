    ...
    private func generateTruthfulCompetitive(message: String, pool: [String], bagKey: String, usedStats: inout Set<String>) -> (String, String?) {
        ...
        if lowered.contains("streak") {
            if let streakDays = Self.extractNumber(from: message, near: ["day", "streak", "consecutive", "straight", "running"]) {
                ...
                var templates = [
                    "\(myDays)-day streak. Just saying.",
                    "Lol \(streakDays) days? I'm at \(myDays).",
                    "\(myDays) days and I haven't missed once.",
                    "Only \(streakDays)? I'm at \(myDays) and still going.",
                    "\(streakDays) days is cool I guess. Mine's \(myDays) though.",
                    "I passed \(streakDays) a while ago. Sitting at \(myDays) now.",
                    "\(myDays)-day streak over here, no big deal.",
                    "Yeah okay but my streak is \(myDays). Come back when you're close.",
                    "\(streakDays)? That's solid but mine's \(myDays) so…",
                    "My \(myDays)-day streak says otherwise.",
                    "I'd be happy with \(streakDays) too if I didn't have \(myDays).",
                    "Bro I was at \(streakDays) like two months ago. \(myDays) now.",
                    "\(myDays) days straight. \(streakDays) is not it.",
                    "You'll get to \(myDays) eventually. Maybe.",
                    "\(streakDays) is a good start. I'm at \(myDays) though.",
                    "Nah \(myDays)-day streak here. Step it up.",
                    "\(streakDays)? Rookie numbers. Hit me up at \(myDays).",
                    "\(myDays) and counting. Catch up.",
                    "That's cute, \(streakDays). Mine's been \(myDays) for a minute now.",
                    "I literally have \(myDays) days. \(streakDays) is nothing to me.",
                ]
        ...
        if lowered.contains("timed") || lowered.contains("challenge") || lowered.contains("speed") {
            if let (mins, secs) = Self.extractTime(from: message) {
                ...
                var templates = [
                    ...
                    "Lol only \(posterTime) left? I had \(myTime).",
                    "\(myTime) left on the clock for me. Just saying.",
                    "Only \(posterTime) remaining? I finished with \(myTime) left.",
                    "\(posterTime) is cutting it close honestly. I had \(myTime) left.",
                    "I used to finish with \(posterTime) too. Now I get \(myTime) left easy.",
                    "\(myTime) left and I wasn't even trying that hard.",
                    "Finished mine with \(myTime) still on the clock.",
                    "\(posterTime) left? That's it? I had \(myTime) so…",
                    "I always have more time left than everyone. \(myTime) today.",
                    "\(posterTime) left is fine I guess. I had \(myTime) though.",
                    "I've been finishing with \(myTime) left for weeks now.",
                    "\(myTime) remaining. Yeah, I don't really struggle with these.",
                    "Had \(myTime) left and honestly could've gone slower.",
                    "\(posterTime) is tight. Mine was \(myTime) left, way more comfortable.",
                    "Finished with \(myTime) on the clock. Wasn't even stressful.",
                    "You had \(posterTime) left? I had \(myTime). It's not the same.",
                    "\(myTime) left over here. These challenges aren't that hard tbh.",
                    "The difference between \(posterTime) and \(myTime) left is just practice.",
                    "I had \(myTime) left on my first try. \(posterTime) is rough.",
                ]
        ...
        if lowered.contains("hall of fame") || lowered.contains("hof") || lowered.contains("infinity") {
            if let infCount = Self.extractNumber(from: message, near: ["infinity", "infinit", "\u{221E}", "\u{00D7}", "count", "entry", "#"]) {
                ...
                var templates = [
                    "I'm at \(myCount) infinities. Just letting you know.",
                    "Lol \(infCount)? I'm at \(myCount).",
                    "\(myCount) infinities over here. Keep grinding though.",
                    "Only \(infCount)? I hit \(myCount) already.",
                    "\(infCount) is a good start. I'm at \(myCount) though.",
                    "I was at \(infCount) a while ago. Sitting at \(myCount) now.",
                    "\(myCount) infinities and still going. Catch up.",
                    "\(infCount) is whatever honestly. I'm at \(myCount).",
                    "\(infCount)? That's solid but mine's \(myCount).",
                    "My infinity count is \(myCount) and it's only going up.",
                    "\(infCount) infinities? Cool I guess. I have \(myCount).",
                    "Bro I passed \(infCount) ages ago. \(myCount) now.",
                    "\(myCount). Yeah, I play a lot.",
                    "You'll get to \(myCount) eventually. Probably.",
                    "\(infCount) is nice. Hit me up when you're at \(myCount) though.",
                    "Nah I'm at \(myCount). \(infCount) isn't really that deep.",
                    "\(infCount)? Rookie numbers. I'm sitting at \(myCount).",
                    "\(myCount) and counting. \(infCount) was a while ago for me.",
                    "That's cute. I remember my first \(infCount). Now I'm at \(myCount).",
                    "I literally have \(myCount) infinities. \(infCount) is nothing to me.",
                ]
        ...
        if let foundIdx = sortedMilestones.firstIndex(where: { message.contains($0) }) {
            ...
            if let originalIdx = Self.allMilestones.firstIndex(of: m),
               originalIdx + 1 < Self.allMilestones.count {
                ...
                var templates = [
                    "I'm at \(higherM). Just saying.",
                    "Lol \(m)? I'm at \(higherM).",
                    "\(higherM) over here. Keep going though.",
                    "Only \(m)? I hit \(higherM) already.",
                    "\(m) is a good start. I'm at \(higherM) though.",
                    "I was at \(m) a while ago. Sitting at \(higherM) now.",
                    "\(higherM) and still climbing. Catch up.",
                    "\(m) is whatever honestly. I'm at \(higherM).",
                    "\(m)? That's solid but mine's \(higherM).",
                    "My highest tile is \(higherM) and it's only going up.",
                    "\(m)? Cool I guess. I'm at \(higherM).",
                    "Bro I passed \(m) ages ago. \(higherM) now.",
                    "\(higherM). Yeah, I play a lot.",
                    "You'll get to \(higherM) eventually. Probably.",
                    "\(m) is nice. Hit me up when you're at \(higherM) though.",
                    "Nah I'm at \(higherM). \(m) isn't really that deep.",
                    "\(m) still? I've been at \(higherM) for a minute.",
                    "\(higherM). Don't worry you'll get there someday.",
                    "I remember when I was at \(m). Good times. I'm at \(higherM) now.",
                    "I'm literally at \(higherM). \(m) was a while ago for me.",
                ]
        ...
        if lowered.contains("quest") {
            ...
            if let posterTierIdx = tiers.firstIndex(where: { message.contains($0) }),
               posterTierIdx < tiers.count - 1 {
                ...
                var templates = [
                    "I'm pulling \(myTier) chests. Just saying.",
                    "Lol \(posterTier)? I get \(myTier).",
                    "\(myTier) chests over here. Keep grinding though.",
                    "Only \(posterTier)? I pull \(myTier) every day.",
                    "\(posterTier) is cool I guess. I'm at \(myTier) though.",
                    "I was getting \(posterTier) a while ago. \(myTier) now.",
                    "\(myTier) chests daily. Not going back to \(posterTier).",
                    "\(posterTier) is whatever honestly. I pull \(myTier).",
                    "\(posterTier)? That's solid but mine's \(myTier).",
                    "My chests are \(myTier) and they keep getting better.",
                    "\(posterTier) chests? Cool. I get \(myTier).",
                    "Bro I was getting \(posterTier) ages ago. \(myTier) now.",
                    "\(myTier). Yeah, the grind pays off.",
                    "You'll get \(myTier) eventually. Probably.",
                    "\(posterTier) is nice. Hit me up when you're pulling \(myTier) though.",
                    "Nah I get \(myTier). \(posterTier) doesn't really compare.",
                    "Still on \(posterTier)? I've been pulling \(myTier) for a while.",
                    "\(myTier) chests hit different. \(posterTier) is whatever.",
                    "I remember opening \(posterTier) chests. Now it's \(myTier) only.",
                    "\(posterTier) is where I started. \(myTier) is where I'm at.",
                ]
