...
public struct MockSocialService: SocialService, Sendable {
    ...
    private func generateDynamicComment(message: String, usedStats: inout Set<String>, forcedTone: String? = nil) -> (commentText: String, nameOverride: String?, tone: String) {
        ...
        let competitiveSymbols = [" >:)", " !!", " !!!", " >", " XD", " RN", " rn", " fr", " tbh", " ngl"]
        ...
        var contextKey = "base"
        if message.contains("Hall of Fame") { contextKey += "_hof" }
        else if message.contains("streak") { contextKey += "_streak" }
        else if message.contains("timed challenge") { contextKey += "_time" }
        else if message.contains("theme") { contextKey += "_theme" }
        else if message.contains("Chest") { contextKey += "_chest" }
        ...
        if Double.random(in: 0...1) < 0.75 {
            ...
            comment = injectSymbol(comment, symbol: symbol)
...
private func generateTruthfulCompetitive(message: String, pool: [String], bagKey: String, usedStats: inout Set<String>) -> (String, String?) {
    ...
    private func generateContextualReply(to commentText: String, message: String, forceTone: String? = nil) -> String {
        ...
        if isAddRequest {
            ...
            if Double.random(in: 0...1) < 0.75 { reply = injectSymbol(reply, symbol: [" >:)", " !!", " !!!", " >", " XD", " RN", " rn", " fr", " tbh", " ngl"].randomElement()!) }
            return reply
        }
        ...
        if isQuestion {
            ...
            if Double.random(in: 0...1) < 0.75 { reply = injectSymbol(reply, symbol: ["?!", ". . .", "??", "!!?"].randomElement()!) }
            return reply
        }
            ...
            if let m = mentionedMilestone {
                ...
                if isLowerBrag, let cM = commentMilestone, let rM = rootMilestone {
                    replies.append(contentsOf: [
                        ...
                        "You're acting like \(cM.name) is a big deal? I effortlessly passed \(rM.name).",
            ...
            if let numStr = mentionedNumber, let num = Int(numStr), !mentionedTime, !mentionedStreak, !mentionedHoF {
                ...
                if isLowerScoreBrag, let cN = commentNumber, let rN = rootNumber {
                    replies.append(contentsOf: [
                        ...
                        "You're acting like \(cN) is a big deal? I effortlessly passed \(rN).",
                ...
                if let numStr = mentionedNumber, let num = Int(numStr) {
                    ...
                    if isLowerStreakBrag, let cN = commentNumber, let rN = rootNumber {
                        replies.append(contentsOf: [
                            ...
                            "You're acting like \(cN) days is a big deal? I effortlessly passed \(rN).",
        ...
        if isJealous {
            ...
            var reply = replies.randomElement()!
            if Double.random(in: 0...1) < 0.75 { reply = injectSymbol(reply, symbol: [" :(", " :((", " >:(", " :/", " ;-(", " -_-", " >_<", ". . .", " rn", " fr", " tbh", " ngl", " smh"].randomElement()!) }
            return reply
        }
        ...
        if isPositive {
            ...
            var reply = replies.randomElement()!
            if Double.random(in: 0...1) < 0.75 { reply = injectSymbol(reply, symbol: ["!!", " :)", " :D", " XD", " xD", " ~", " :P", " <3", " =)", " ^_^", " rn", " RN", " fr", " tbh"].randomElement()!) }
            return reply
        }
