...
Open `game2244.xcworkspace` for the full workspace. Runtime code lives in `2244/game2244/` (app target) and the modular SwiftPM packages under `Packages/` (`GameCore`, `GameApp`, `GameUI`, `GameServices`, `GameTestingSupport`, `GlassPreview`). UI tests and integration flows sit in `2244/game2244UITests/`, while Swift Testing specs for each module live in the matching `Packages/*/Tests` folders. Shared docs reside in `Docs/`, automation helpers in `scripts/`, and Firebase assets in `firebase/`.

## Build, Test, and Development Commands
Use Xcode or the CLI to build: `FIREBASE_SOURCE_FIRESTORE=1 xcodebuild -workspace game2244.xcworkspace -scheme game2244 -destination 'platform=iOS Simulator,name=iPhone 16' build` for simulator runs, and add `-configuration Release -sdk iphoneos archive` for device-ready artifacts. Keep `FIREBASE_SOURCE_FIRESTORE=1` when resolving or archiving the app target so the committed Xcode lockfiles retain the `grpc-ios` source-Firestore graph required by Xcode Cloud. Execute module tests with `swift test --package-path Packages/GameCore` (repeat for other packages) when iterating quickly. Run the end-to-end suite via `FIREBASE_SOURCE_FIRESTORE=1 xcodebuild test -workspace game2244.xcworkspace -scheme game2244 -destination 'platform=iOS Simulator,name=iPhone 16'`. Invoke `scripts/create-new-feature.sh` to scaffold feature plans when collaborating with other agents.
